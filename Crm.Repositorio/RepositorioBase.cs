...
namespace Crm.Repositorio
{
    ...
    public partial class RepositorioBase<EntityType> : IDisposable where EntityType : Entity
    {
        ...
        /// <returns></returns>
        public Guid Create(Entity registro, Guid userId)
        {
            lock (syncRoot)
            {
               Contexto.Proxy.CallerId = userId;
                    try
                    {
        ...
        public void Delete(string entityName, Guid id, Guid userId)
        {
            lock (syncRoot)
            {
                Contexto.Proxy.CallerId = userId;
                try
                {
                    Contexto.Proxy.Delete(entityName, id);
                }
                catch
                {
                    throw;
        ...
        /// <returns></returns>
        public EntityCollection RetrieveMultiple(ColumnSet atributos, params ConditionExpression[] condicoes)
        {
